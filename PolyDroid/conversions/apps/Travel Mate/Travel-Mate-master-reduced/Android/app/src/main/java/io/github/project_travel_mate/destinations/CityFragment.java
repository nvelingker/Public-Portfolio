...
public class CityFragment extends Fragment implements TravelmateSnackbars {
    ...
    private void cityAutoComplete() {
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Log.e("AC Request Failed", "Message : " + e.getMessage());
            }
    ...
    public void fetchCitiesList() {
        try {
            SharedPreferences getSharedPreferences = getActivity().getSharedPreferences
                    ("CityFragment", Context.MODE_PRIVATE);
            String tojson = getSharedPreferences.getString(
                    "io.github.project_travel_mate.citieslist",
                    "0");
            if (!tojson.equals("0")) {
                Gson gson = new Gson();
                String json = getSharedPreferences.
                        getString("io.github.project_travel_mate.citieslist", "");
        ...
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Log.e("FC Requests Failed", "Message : " + e.getMessage());
                    ...
                    if (response.isSuccessful()) {
                        try {
                            ...
                            SharedPreferences sharedPreferences = getActivity().getSharedPreferences
                                    ("CityFragment", Context.MODE_PRIVATE);
                            Gson gson = new Gson();
                            String citiestojson = gson.toJson(cities);
                            sharedPreferences.edit()
                                    .putString("io.github.project_travel_mate.citieslist",
                                             citiestojson).apply();
    ...
    private void setListener(List<City> cities) {
        lv.setAdapter(new CityAdapter(mActivity, cities, new FlipSettings.Builder().defaultPage().build()));
